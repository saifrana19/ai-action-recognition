...
def run(
    ...
    # Initialize video capture with YouTube URL handling
    if source.startswith("http") and urlparse(source).hostname in {"www.youtube.com", "youtube.com", "youtu.be"}:
        try:
            # Try the new ultralytics YouTube handling
            from ultralytics.utils.downloads import get_best_youtube_url
            ...
            try:
                ...
                yt = YouTube(source)
                source = yt.streams.filter(progressive=True, file_extension='mp4').order_by('resolution').desc().first().url
                print(f"Using YouTube stream: {source}")
            except ImportError:
                print("Warning: Could not process YouTube URL properly. Install pytube: pip install pytube")
